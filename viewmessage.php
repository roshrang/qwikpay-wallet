...
<div class="agileits-inner-banner">
	...
	<div class="w3layouts-breadcrumbs text-center">
		<div class="container">
			<span class="agile-breadcrumbs"><a href="index.php"><i class="fa fa-home home_1"></i></a> / <a href="account.php"><span>Account</span></a>/ <span>ViewMessage</span></span>
		</div>
	</div>
	...
	<div class="resp-tabs-container hor_1">
                    ...
                    <i class="icon fa fa-user inner-user" aria-hidden="true"></i>
										...
										<?php
										$msg_id=$_GET['msg'];
										$msg_type=substr($msg_id,-1);
										$msg_id=substr($msg_id,2,-1);
										if($msg_type==1)
										{
											$stmt = $conn->prepare("SELECT `msg_id`,`msg_sub`,`msg_des`,`msg_date`,msg_sts,usr_name FROM `msg_details` MD inner join user_data UD on MD.susr_id=UD.usr_id where MD.msg_id=?");
										
											if (!$stmt->bind_param("i", $msg_id)) {
...
while($row = $stmt->fetch())
		  {?>
						<h4>Received From:</h4><br>
						<p><?php echo $usr_name;?></p><br>
						<h4>Subject:</h4><br>
						<p><?php echo $msg_sub;?></p><br>
						<h4>Received On</h4><br>
						<p><?php echo $msg_date;?></p><br>
						<h4>Description</h4><br>
						<p><?php echo $msg_des;?></p>
					</div>
			     <div class="clearfix"> </div>
				 ...
				 <?php
										...
										if($msg_type==2)
										{
											$stmt = $conn->prepare("SELECT `msg_id`,`msg_sub`,`msg_des`,`msg_date`,msg_sts,usr_name FROM `msg_details` MD inner join user_data UD on MD.rusr_id=UD.usr_id where MD.msg_id=?");
											if (!$stmt->bind_param("i", $msg_id)) {
    echo "Binding parameters failed: (" . $stmt->errno . ") " . $stmt->error;
}
