...
<div class="agileits-inner-banner">
	...
	<div class="w3layouts-breadcrumbs text-center">
		<div class="container">
			<span class="agile-breadcrumbs"><a href="index.php"><i class="fa fa-home home_1"></i></a> / <a href="account.php"><span>Account</span></a>/ <span>Outbox</span></span>
		</div>
	</div>
	...
	<!--bus-single --> 
            <div class="w3agile single-bus">
			         <h3 class="w3-head">Outbox</h3>
			 <!-- train-routes -->
					<div class="bus-tp">
						<div class="bus-tp-inner">
							<h3>Last 10 Sent Messages</h3>
							<div class="clearfix"></div>
						</div>
					</div>
				<!-- /train-routes -->
				...
				<?php
				$user_id=$_SESSION['usr_id'];
//$stmt = $conn->prepare("SELECT `msg_id`,`rusr_id`,`msg_sub`,`msg_des`,`msg_date`,`msg_sts` FROM `msg_details` WHERE `susr_id`=?");
$stmt = $conn->prepare("SELECT `msg_id`,`msg_sub`,`msg_des`,`msg_date`,msg_sts,usr_name FROM `msg_details` MD inner join user_data UD on MD.rusr_id=UD.usr_id where MD.susr_id=?");
...
$stmt->store_result(); 
        ...
        ?>
		     <tr>
				...
				<td class="wthree"><i class="glyphicon glyphicon-ok"></i><?php echo $msg_des; ?></td> 
				<td class="wthree"><i class="fa fa-clock-o"></i><?php echo substr($msg_date, 0, 10); ?></td>
				<td class="wthree"><i class="glyphicon glyphicon-ok"></i><?php echo $msg_sts; ?></td>
				<td class="wthree"><a href="viewmessage.php?msg=<?php echo "14".$msg_id."2";?>" data-toggle="modal" data-target="#myModalbook" class="seat-button two">View Message</a></td>
			 </tr>
			 <?php
		  }
	 }
